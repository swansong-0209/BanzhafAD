...
Semisupervise_AD_Pool = ['Left_STAMPi', 'SAND', 'MCD', 'Sub_MCD', 'OCSVM', 'Sub_OCSVM', 'AutoEncoder', 'CNN', 'LSTMAD', 'TranAD', 'USAD', 'OmniAnomaly', 'PatchTST',
                        'AnomalyTransformer', 'TimesNet', 'FITS', 'Donut', 'OFA', 'MOMENT_FT', 'M2N2', 'TSPulse_FT', 'xLSTMAD', 'CHARM', 'KANAD', 'BanzhafAD']
...
def run_Time_RCD(data,
    ...
    # Multi-channel: process channel-by-channel with uni checkpoint, then take point-wise maximum
    clf = Time_RCD(
        win_size=win_size,
        input_c=1,
        batch_size=batch_size,
    ...
    for i in range(n_features):
        clf.zero_shot(data[:, i:i + 1])
        scores.append(clf.decision_scores_.ravel())
...
def run_KANAD(
    ...
    from .models.KANAD import KANAD_AD
...
def run_BanzhafAD(data_train, data_test, window_size=100, patch_len=10, reduce_channels=5, num_samples=50, n_prototypes='auto'):
    from .models.BanzhafAD import BanzhafAD
    clf = BanzhafAD(win_size=window_size, patch_len=patch_len, reduce_channels=reduce_channels, num_samples=num_samples, n_prototypes=n_prototypes)
    clf.fit(data_train)
    score = clf.decision_function(data_test)
    return score.ravel()
